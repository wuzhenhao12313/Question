...
const attack = ([r, x1, y1, x2, y2, x3, y3, x0, y0]) => {
    let output=0;
    let [d1,d2,d3]=[countDistance([x1, y1], [x0, y0]),
                    countDistance([x2, y2], [x0, y0]),
                    countDistance([x3, y3], [x0, y0])];
    [d1, d2, d3].map((i)=>{
       if (r>=i){
           output+=1;
       }
...
const countDistance = ([x1, y1], [x2, y2]) => {
    ...
    let y = y1 >= y2 ? y1 - y2 : y1 - y2;

    distance = Math.sqrt(x * x + y * y);
    return distance;
}

let input = [1, 1, 1, 2, 2, 3, 3, 1, 2];
